...
import ICRC "./ICR/ICRC";

actor class Main(_ckBTCPrincipal : Principal, admin : Principal, free_NFTCanister : Principal) = this {
  ...
  private func _createArtRights(caller : Principal, subaccount : ?Blob, name : Text, symbol : Text, maxLimit : Nat16, mintPrice : Nat) : async Nat {
    let new_art_canister = await Dip721.Dip721NFT(
  ...
  public shared ({ caller }) func createArtRights(subaccount : ?Blob, name : Text, symbol : Text, maxLimit : Nat16, mintPrice : Nat) : async {
    collectionID : Nat;
  } {
    return {
      collectionID = await _createArtRights(caller, subaccount, name, symbol, maxLimit, mintPrice);
    };
  };
  ...
  public shared ({ caller }) func mintCollection(collectionID : Nat, subaccount : ?Blob) : async TxError {
